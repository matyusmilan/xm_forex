...
from demo_app.models import Order, OrderStatus
...
class TestApp:
    ...
    def test_get_root(self, app_service):
        """
    ...
    def test_get_orders(self, app_service):
        ...
        WHEN /orders endpoint is called with POST method (order_input_1)
        THEN response with status 201
        WHEN /orders endpoint is called with GET method
        THEN response with status 200
        AND data of response has one element and equals the order_input_1
        WHEN /orders endpoint is called with POST method (order_input_2)
        THEN response with status 201
        WHEN /orders endpoint is called with GET method
        THEN response with status 200
        AND data of response has two elements and equals the inserted orders
        """
        # empty
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(1, 1, 200, [1], id="one_element"),
            pytest.param(3, 1, 200, [], id="empty_result"),
            pytest.param(1, 0, 200, [], id="zero_limit"),
            pytest.param(1, 2, 200, [1, 2], id="two_element"),
        ],
    )
    def test_get_orders_paging(
        self, app_service, offset, limit, status_code, expected_result
    ):
        """
        GIVEN The API is running (fixture)
        AND DB is empty
        WHEN /orders endpoint is called 3 times with POST method
        WHEN call /orders endpoint with GET method (offset=1, limit 1) [id=one_element]
        THAN response with status 200 [id=one_element]
        AND the list contains the 2nd inserted order [id=one_element]
        WHEN call /orders endpoint with GET method (offset 3, limit 1) [id=empty_result]
        THAN response with status 200 [id=empty_result]
        AND the response is an empty list [id=empty_result]
        WHEN call /orders endpoint with GET method (offset 1,limit 0) [id=zero_limit]
        THAN response with status 200 [id=zero_limit]
        AND the response is an empty list [id=zero_limit]
        WHEN call /orders endpoint with GET method (offset 1, limit 2) [id=two_element]
        THAN response with status 200 [id=two_element]
        AND the list contains the 2nd and 3rd inserted orders [id=two_element]
        """
        full_result = []
    ...
    def test_get_orders_paging_invalid_input(
        self, app_service, offset, limit, status_code, error_msg
    ):
        """
        GIVEN The API is running (fixture)
        WHEN /orders endpoint is called with GET method with offset "apple" and limit 1 [id=offset_is_string]
        THAN response with status 422 [id=offset_is_string]
        AND message  in response is the expected error message [id=offset_is_string]
        WHEN /orders endpoint is called with GET method with offset 1 and limit "apple" [id=limit_is_string]
        THAN response with status 422 [id=limit_is_string]
        AND message  in response is the expected error message [id=limit_is_string]
